...
namespace Microsoft.Internal.Common.Utils
{
    ...
    internal sealed class ProcessLauncher
    {
        ...
        public bool Start(string diagnosticTransportName, CancellationToken ct, bool showChildIO, bool printLaunchCommand)
        {
            ...
            {
                throw new Exception($"An error occurred trying to start process '{_childProc.StartInfo.FileName}' with working directory '{System.IO.Directory.GetCurrentDirectory()}'. {e.Message}");
            }
            if (!showChildIO)
            {
